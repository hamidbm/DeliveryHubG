...
export async function POST(request: Request) {
  try {
    ...
    const hasOpenAiKey = Boolean(process.env.OPENAI_API_KEY || aiSettings.openaiKey);
    const providerSetting = aiSettings.defaultProvider || (settings as any)?.defaultProvider;
    const rawProvider = String(providerSetting || '').trim().toUpperCase();
    const isExplicitGemini = rawProvider.includes('GEMINI');
    const isOpenAiAlias = rawProvider.includes('OPENAI') || rawProvider.includes('CHATGPT') || rawProvider.includes('GPT');
    const provider = isExplicitGemini
      ? 'GEMINI'
      : isOpenAiAlias || hasOpenAiKey
        ? 'OPENAI'
        : 'GEMINI';
    const model =
