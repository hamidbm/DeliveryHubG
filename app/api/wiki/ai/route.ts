...
export async function POST(request: Request) {
  try {
    ...
    const rawProvider = String(providerSetting || 'GEMINI').trim().toUpperCase();
    const provider = rawProvider.includes('OPENAI') || rawProvider.includes('CHATGPT') || rawProvider.includes('GPT')
      ? 'OPENAI'
      : rawProvider.includes('GEMINI')
        ? 'GEMINI'
        : rawProvider;
    const model =
