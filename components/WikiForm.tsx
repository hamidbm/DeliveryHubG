...
// Props interface for the WikiForm component
...
/**
 * WikiForm Component
 * Handles editing of existing Wiki artifacts.
 */
const WikiForm: React.FC<WikiFormProps> = ({ 
  ...
  // Helper to insert markdown or HTML tags into the editor
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        onSaveSuccess(id || data.result?.insertedId || '');
      } else {
      ...
      <header className="px-10 py-5 bg-white border-b border-slate-200 flex items-center justify-between shadow-sm shrink-0">
        ...
        <div className="flex items-center gap-4">
          ...
          >
            ...
            {isSaving ? 'Syncing Registry' : 'Save Changes'}
          </button>
        </div>
      </header>
...
const ToolbarButton = ({ icon, label, onClick }: any) => (
  <button onClick={onClick} title={label} className="w-10 h-10 flex items-center justify-center text-slate-500 hover:text-blue-600 hover:bg-white border border-transparent hover:border-slate-100 hover:shadow-lg rounded-xl transition-all">
...
const TemplateCard = ({ title, desc, onClick }: any) => (
  <button onClick={onClick} className="w-full text-left p-4 bg-white border border-slate-200 rounded-2xl hover:border-blue-400 hover:shadow-xl transition-all group">
