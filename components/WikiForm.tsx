...
const WikiForm: React.FC<WikiFormProps> = ({ 
  ...
  const toolbarButtons = [
    { label: 'Bold', icon: 'fa-bold', action: () => insertText('**', '**') },
    { label: 'Italic', icon: 'fa-italic', action: () => insertText('_', '_') },
    { label: 'Heading 1', icon: 'fa-heading', action: () => insertText('# ', '') },
    { label: 'Heading 2', icon: 'H2', action: () => insertText('## ', '') },
    { label: 'List', icon: 'fa-list-ul', action: () => insertText('- ', '') },
    { label: 'Code', icon: 'fa-code', action: () => insertText('```\n', '\n```') },
    { label: 'Quote', icon: 'fa-quote-left', action: () => insertText('> ', '') },
    { label: 'Internal Wiki Link', icon: 'fa-book-bookmark', action: () => insertText('[[', '') },
  ];

  return (
    <div className="flex flex-col h-full animate-fadeIn bg-white relative">
      <div className="px-10 py-4 border-b border-slate-100 flex items-center justify-between bg-white sticky top-0 z-30 shadow-sm">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-blue-600 text-white flex items-center justify-center shadow-sm">
            <i className="fas fa-pen-nib text-xs"></i>
          </div>
          <div className="flex flex-col">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Editing Node</span>
            <span className="text-xs font-bold text-slate-800 truncate max-w-[200px]">{title || 'Draft Document'}</span>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button 
            onClick={onCancel}
            className="px-4 py-2 text-[10px] font-black text-slate-400 hover:text-slate-800 transition-all uppercase tracking-widest rounded-xl hover:bg-slate-50"
          >
            Discard
          </button>
          <button 
            ...
            className="px-8 py-2 bg-slate-900 text-white text-[10px] font-black rounded-xl shadow-xl shadow-slate-900/10 hover:bg-slate-800 disabled:opacity-30 transition-all uppercase tracking-widest flex items-center gap-2"
          >
      ...
      <div className="px-10 py-3 border-b border-slate-50 bg-slate-50/50 flex flex-wrap items-center gap-6 sticky top-[65px] z-20">
        <div className="flex items-center gap-1">
          {toolbarButtons.map((btn, idx) => (
            <button
              ...
              onClick={btn.action}
              title={btn.label}
              className="w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 hover:text-blue-600 hover:bg-white hover:shadow-sm transition-all text-xs"
              disabled={viewMode === 'preview'}
            >
              {btn.icon.length <= 2 ? <span className="font-black">{btn.icon}</span> : <i className={`fas ${btn.icon}`}></i>}
            </button>
          ))}
        </div>
        ...
        <div className="flex items-center gap-4 flex-wrap">
          <div className="flex items-center gap-2">
            <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Parent Registry:</label>
            <select 
              value={parentId}
            ...
            >
              ...
              {allPages.filter(p => (p._id || p.id) !== id).map(p => (
          ...
          <div className="p-12 md:p-16 lg:p-24 max-w-5xl mx-auto w-full">
            <div className="space-y-10">
              <input 
                ...
                className="text-5xl font-black text-slate-900 w-full border-none focus:ring-0 p-0 placeholder:text-slate-100 tracking-tighter bg-transparent"
              ...
              <div className="relative">
                ...
                <textarea 
                  ...
                  className="w-full min-h-[600px] border-none focus:ring-0 p-0 text-slate-600 leading-relaxed resize-none text-xl placeholder:text-slate-100 font-medium bg-transparent"
                  placeholder="Capture findings... Tip: Type [[ to link internal documents."
                />
