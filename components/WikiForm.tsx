...
const WikiForm: React.FC<WikiFormProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-50 flex flex-col animate-fadeIn">
          ...
          <div className="px-8 py-3 border-b border-slate-100 bg-slate-50 flex items-center justify-between sticky top-0 z-10">
            <div className="flex items-center gap-1">
              ...
              <div className="flex items-center gap-1.5 ml-2">
                {COLORS.map(c => (
                  <button key={c.name} onClick={() => insertText(`<span style="color: ${c.value}">`, '</span>')} className="w-6 h-6 rounded-full hover:scale-125 transition-all shadow-sm border border-black/5" style={{ backgroundColor: c.value }} title={c.name}></button>
                ))}
              </div>
            </div>
      ...
      <style jsx>{` .custom-scrollbar::-webkit-scrollbar { width: 5px; } .custom-scrollbar::-webkit-scrollbar-track { background: transparent; } .custom-scrollbar::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 10px; } `}</style>
    </div>
  );
};
