...
const WorkItemDetails: React.FC<WorkItemDetailsProps> = ({ item: initialItem, bundles, applications, onUpdate, onClose }) => {
  ...
  // Presence Logic (Mocked for Enterprise feel)
  const [activeCollaborators, setActiveCollaborators] = useState<string[]>(['Alex Architect']);
  ...
  useEffect(() => {
    ...
    // Simulate random other presence
  ...
  const handleUpdateItem = async (updates: Partial<WorkItem>) => {
    ...
    // Governance Guardrail: Prevent closure if checklist is incomplete
           ...
           <div className="flex items-center -space-x-2">
              {activeCollaborators.map((name, i) => (
                <div key={i} className="relative group/user">
                  <div className="w-8 h-8 rounded-full border-2 border-white bg-slate-100 flex items-center justify-center text-[10px] font-bold text-slate-500 shadow-sm relative overflow-hidden">
                  ...
                  <div className="absolute top-full right-0 mt-2 bg-slate-900 text-white text-[8px] px-2 py-1 rounded-md font-black uppercase tracking-widest opacity-0 group-hover/user:opacity-100 transition-opacity z-50 whitespace-nowrap shadow-xl">
                    {name === 'Current User' ? 'You' : name} (Viewing)
                  </div>
                </div>
              ))}
           </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar bg-[#FAFAFA]">
        {closureError && (
          <div className="mx-10 mt-8 p-5 bg-red-50 border border-red-200 rounded-[1.5rem] flex items-center gap-4 text-red-600 animate-fadeIn shadow-lg shadow-red-500/5">
             ...
             <div className="flex-1"><p className="text-[10px] font-black uppercase tracking-[0.15em]">Governance Violation</p><p className="text-sm font-bold">{closureError}</p></div>
          ...
          <div className="p-10 space-y-10 animate-fadeIn">
            <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm space-y-8">
              <div className="grid grid-cols-2 gap-8">
                <DetailField label="Workflow Phase">
                    <select 
                      ...
                      className={`w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-sm font-black outline-none focus:ring-2 focus:ring-blue-500/20 transition-all ${item.status === WorkItemStatus.DONE ? 'text-emerald-600' : 'text-slate-700'}`}
                    >
              ...
              <div className="grid grid-cols-3 gap-8">
                ...
                <DetailField label="Estimate (Pts)"><input type="number" value={item.storyPoints || 0} onChange={(e) => handleUpdateItem({ storyPoints: parseInt(e.target.value) || 0 })} className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-sm font-black outline-none" /></DetailField>
                <DetailField label="Effort Logged">
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => setIsLoggingWork(true)} className="w-12 h-12 rounded-2xl bg-blue-600 text-white flex items-center justify-center hover:bg-blue-700 shadow-lg shadow-blue-500/20 active:scale-95 transition-all"><i className="fas fa-plus"></i></button>
                  </div>
                </DetailField>
              </div>
            </div>
            ...
            <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden divide-y divide-slate-50">
               {(item.checklists || []).map(c => (
                 <div key={c.id} className="flex items-center gap-4 p-5 hover:bg-slate-50/50 transition-colors group">
                    <button onClick={() => toggleChecklistItem(c.id)} className={`w-8 h-8 rounded-xl border-2 flex items-center justify-center transition-all ${c.isCompleted ? 'bg-blue-600 border-blue-600 text-white' : 'border-slate-200 bg-white'}`}>
                       {c.isCompleted && <i className="fas fa-check text-xs"></i>}
                    </button>
                    <span className={`flex-1 text-sm font-bold transition-all ${c.isCompleted ? 'text-slate-400 line-through' : 'text-slate-700'}`}>{c.label}</span>
               ...
               <div className="p-4 bg-slate-50/30 flex items-center gap-3">
                  ...
                  <button onClick={handleAddChecklistItem} className="w-10 h-10 rounded-xl bg-white border border-slate-200 text-blue-600 shadow-sm hover:shadow-md transition-all flex items-center justify-center"><i className="fas fa-plus text-xs"></i></button>
               </div>
            </div>
          </div>
        )}
