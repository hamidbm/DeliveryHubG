...
const WorkItemDetails: React.FC<WorkItemDetailsProps> = ({ item: initialItem, bundles, applications, onUpdate, onClose }) => {
  ...
  useEffect(() => {
    ...
    setActiveCollaborators(['Current User', 'Alex Architect']);
  }, [initialItem]);
  ...
  const handleUpdateItem = async (updates: Partial<WorkItem>) => {
    ...
    if (updates.status === WorkItemStatus.DONE) {
      if ((item.checklists || []).some(c => !c.isCompleted)) {
        setClosureError(`Governance Violation: DoD Checklist items must be verified before closure.`);
        return;
      }
    }
    setSaving(true);
  ...
  return (
    <div className="h-full flex flex-col bg-white overflow-hidden relative shadow-2xl border-l border-slate-200">
      <header className="px-10 py-8 border-b border-slate-100 flex items-center justify-between shrink-0 bg-slate-50/20">
        ...
        <div className="flex items-center gap-6 shrink-0">
           ...
           <div className="flex items-center gap-2">
              ...
              <button onClick={() => handleUpdateItem({ watchers: (item.watchers || []).includes('User') ? [] : ['User'] })} className={`w-10 h-10 rounded-xl border flex items-center justify-center transition-all ${(item.watchers || []).length ? 'bg-amber-50 border-amber-200 text-amber-500' : 'bg-white border-slate-200 text-slate-300'}`}><i className="fas fa-eye"></i></button>
           </div>
        </div>
      </header>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar bg-[#FAFAFA]">
        {closureError && (
          <div className="mx-10 mt-8 p-5 bg-red-50 border border-red-200 rounded-[1.5rem] flex items-center gap-4 text-red-600 animate-fadeIn shadow-lg shadow-red-500/5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
                  <div className="flex items-center gap-4">
                     ...
                     <h4 className="font-black text-slate-800 uppercase tracking-tight">Technical Roadmapper</h4>
                  </div>
                  <p className="text-sm text-slate-500 font-medium leading-relaxed">AI analysis of requirements into implementation steps and test cases.</p>
                  <button onClick={handleAiRefinement} disabled={aiLoading} className="w-full py-4 bg-slate-900 text-white text-[10px] font-black rounded-2xl uppercase tracking-widest hover:bg-slate-800 transition-all disabled:opacity-50">
                    {aiLoading ? <i className="fas fa-sparkles fa-spin"></i> : 'Execute Scope Analysis'}
                  </button>
               </div>
               ...
               <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
                  ...
                  <p className="text-sm text-slate-500 font-medium leading-relaxed">Converts registry activity feed into concise executive status bullets.</p>
                  <button onClick={handleGenerateStandupDigest} disabled={isSummarizing} className="w-full py-4 bg-blue-600 text-white text-[10px] font-black rounded-2xl uppercase tracking-widest hover:bg-blue-700 transition-all disabled:opacity-50">
                    {isSummarizing ? <i className="fas fa-circle-notch fa-spin"></i> : 'Generate Sync Digest'}
                  </button>
               </div>
            </div>

            {standupDigest && (
              <div className="bg-blue-50/50 p-10 rounded-[3rem] border border-blue-100 shadow-sm animate-fadeIn relative">
                <div className="absolute top-6 right-10 text-blue-300 opacity-20"><i className="fas fa-quote-right text-4xl"></i></div>
                <h4 className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-6 border-b border-blue-200 pb-2 inline-block">Standup Protocol</h4>
                <div className="prose prose-blue prose-sm max-w-none font-medium text-slate-700" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(marked.parse(standupDigest) as string) }} />
              </div>
            )}

            {aiPlan && (
              <div className="bg-white p-12 rounded-[3rem] border border-slate-100 shadow-sm prose prose-slate max-w-none animate-fadeIn" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(marked.parse(aiPlan) as string) }} />
            )}
          </div>
        )}
               ...
               <div className="bg-white border-2 border-slate-100 rounded-[2.5rem] p-6 flex flex-col shadow-inner focus-within:border-blue-400 transition-all">
                 ...
                 <div className="flex justify-end mt-4">
                    <button onClick={async () => {
                       ...
                       setNewComment('');
                    }} className="px-10 py-3 bg-blue-600 text-white text-[10px] font-black uppercase rounded-2xl hover:bg-blue-700 shadow-xl active:scale-95">Post Logic</button>
                 </div>
               </div>
            </div>
          </div>
        ...
        {activeTab === 'links' && (
          <div className="p-10 space-y-10 animate-fadeIn">
            <header className="flex justify-between items-center bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm relative overflow-hidden">
              ...
              <div>
                <h4 className="text-xl font-black text-slate-800 tracking-tight">Downstream Impact Network</h4>
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">This node restricts <span className="text-red-500">{downstreamImpact}</span> critical dependencies.</p>
              </div>
            ...
            <div className="grid grid-cols-1 gap-6">
               ...
               {(!item.links || item.links.length === 0) && (
                  <div className="py-32 text-center bg-slate-50/50 border-2 border-dashed border-slate-100 rounded-[3rem] text-slate-300">
                     ...
                     <p className="text-[10px] font-black uppercase tracking-widest">No topological relationships established</p>
                  </div>
               )}
            </div>
          </div>
          ...
          <div className="p-10 space-y-10 animate-fadeIn">
            <div className="flex justify-between items-center bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
              <div>
                ...
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Archived technical exports and specifications.</p>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                {(!item.attachments || item.attachments.length === 0) && (
                  <div className="col-span-full py-32 text-center bg-slate-50/50 border-2 border-dashed border-slate-100 rounded-[3rem]">
                     ...
                     <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Repository empty. Archive artifacts for audit.</p>
                  </div>
                )}
            </div>
          </div>
        ...
        {activeTab === 'activity' && (
          <div className="p-10 space-y-6 animate-fadeIn bg-white/50">
             <header className="flex justify-between items-center mb-12 px-4">
                <div>
                   <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Deployment Logic Trace</h4>
                   <p className="text-lg font-black text-slate-800 mt-1">Immutable record of execution history.</p>
                </div>
      ...
      {isLinking && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-md z-[300] flex items-center justify-center p-6">
           <div className="bg-white rounded-[3rem] w-full max-w-lg p-12 shadow-2xl animate-fadeIn border border-slate-100">
              <h3 className="text-3xl font-black text-slate-900 mb-8 tracking-tighter italic">Connect Logic Node</h3>
              <div className="space-y-8">
                 <div className="grid grid-cols-2 gap-6">
                   ...
                   <DetailField label="Registry Lookup">
        ...
        <div className="fixed inset-0 z-[500] bg-slate-950/95 backdrop-blur-2xl flex flex-col animate-fadeIn">
          ...
          <main className="flex-1 p-12 flex items-center justify-center overflow-hidden">
             <div className="w-full h-full max-w-7xl bg-white rounded-[4rem] shadow-[0_50px_100px_rgba(0,0,0,0.5)] overflow-hidden relative border border-white/20">
                {viewingAttachment.type.includes('image') ? <img src={viewingAttachment.url} className="w-full h-full object-contain" /> : <iframe src={viewingAttachment.url} className="w-full h-full border-none" title="Artifact Protocol Viewer" />}
             </div>
          </main>
        </div>
      )}
