...
const WorkItemsBoardView: React.FC<WorkItemsBoardViewProps> = ({ 
  ...
  const fetchSprints = async () => {
    ...
    if (active && activeSprintId === 'all') setActiveSprintId(active._id!);
  };
  ...
  useEffect(() => {
    fetchBoard();
  }, [selBundleId, selAppId, selMilestone, selEpicId, searchQuery, activeSprintId]);
  ...
  const swimlaneRows = useMemo(() => {
    ...
    allItems.forEach(item => {
      const key = groupBy === 'assignee' ? (item.assignedTo || 'Unassigned') : (item.parentId ? `EPIC: ${item.parentId}` : 'No Epic');
         ...
         <div className="flex items-center gap-6">
            <div className="flex items-center gap-2 bg-slate-100 p-1 rounded-xl">
               ...
               <select 
                ...
                className="bg-transparent text-[10px] font-bold outline-none pr-2"
               >
              ...
              {swimlaneRows?.map(([rowName, rowItems]) => (
                <div key={rowName} className="space-y-4">
                   <div className="flex items-center gap-4 px-4">
                      <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] bg-white border border-slate-200 px-4 py-1 rounded-full shadow-sm">{rowName}</span>
                      <div className="h-[1px] flex-1 bg-slate-100"></div>
                   </div>
                   <div className="flex gap-6">
                      {boardData.columns.map(col => (
                        <KanbanColumn key={`${rowName}-${col.statusId}`} column={{ ...col, items: rowItems.filter(i => i.status === col.statusId) }} onItemClick={setActiveItem} hideHeader />
                      ))}
                   </div>
                </div>
              ))}
