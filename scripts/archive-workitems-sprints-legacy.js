...
async function main() {
  ...
  const legacyName = 'sprints';
  const legacyExists = await db.listCollections({ name: legacyName }, { nameOnly: true }).hasNext();
  if (!legacyExists) {
    console.log('No legacy sprints collection found. Nothing to archive.');
    await client.close();
    return;
  }
  ...
  if (!legacyCount) {
    console.log('Legacy sprints is empty. Nothing to archive.');
    await client.close();
    return;
  }

  const archiveName = `workitems_sprints_archive_${timestampTag()}`;
  await db.collection(legacyName).rename(archiveName);
  console.log(`Archived sprints -> ${archiveName}`);
