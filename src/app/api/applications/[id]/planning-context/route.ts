...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json(buildPlanningContextPayload(normalizedBundle, normalizedApp, resolved));
  } catch (error: any) {
    return NextResponse.json({ error: error.message || 'Failed to load planning context' }, { status: 500 });
  }
}
