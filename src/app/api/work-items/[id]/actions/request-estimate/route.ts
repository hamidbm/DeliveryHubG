...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const reason = body?.reason ? String(body.reason) : 'Request estimate for critical path item.';
    ...
    await createNotificationsForEvent({
      type: 'workitem.estimate.requested',
      actor,
    ...
    await emitEvent({
      ...
      payload: { actionType: 'REQUEST_ESTIMATE', reason }
    });

    return NextResponse.json({ success: true });
  } catch (err: any) {
    return NextResponse.json({ error: err?.message || 'Failed to request estimate' }, { status: 500 });
  }
}
