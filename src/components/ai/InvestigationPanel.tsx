...
const InvestigationPanel: React.FC<{
  ...
  onWatch?: (item: SavedInvestigation) => void;
}> = ({ items, busyId, onRun, onRefresh, onTogglePin, onDelete, onWatch }) => {
  return (
          ...
          return (
            <div key={item.id} className="border border-slate-200 rounded-lg p-3 bg-slate-50">
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                ...
                {onWatch && (
                  <button disabled={busy} onClick={() => onWatch(item)} className="px-2 py-1 rounded border border-slate-300 bg-white text-xs font-semibold text-slate-700 hover:bg-slate-100 disabled:opacity-60">Watch</button>
                )}
