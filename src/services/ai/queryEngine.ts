import { PortfolioQueryEvidenceItem, PortfolioQueryResponse, StructuredPortfolioReport } from '../../types/ai';
...
const buildDefaultEvidence = (signals: PortfolioSignalSummary): PortfolioQueryEvidenceItem[] => [
  { label: 'Total Work Items', value: `${signals.totalWorkItems}` },
  { label: 'Unassigned Work', value: `${signals.unassignedWorkItems} (${(signals.unassignedRatio * 100).toFixed(1)}%)` },
  { label: 'Blocked Work', value: `${signals.blockedWorkItems} (${(signals.blockedRatio * 100).toFixed(1)}%)` },
  { label: 'Overdue Work', value: `${signals.overdueWorkItems} (${(signals.overdueRatio * 100).toFixed(1)}%)` },
  { label: 'Open Reviews', value: `${signals.reviewsOpen}` },
  { label: 'Overdue Milestones', value: `${signals.milestonesOverdue}` }
];
...
export const answerPortfolioQuestionDeterministically = (
  ...
  if (q.includes('unassigned')) {
    return {
      ...
      evidence: [
        { label: 'Unassigned Work', value: `${signals.unassignedWorkItems}` },
        { label: 'Unassigned Ratio', value: `${(signals.unassignedRatio * 100).toFixed(1)}%` }
      ],
      followUps: [
  ...
  if (q.includes('milestone') && q.includes('risk')) {
    ...
    return {
      ...
      evidence: [
        { label: 'Overdue Milestones', value: `${signals.milestonesOverdue}` },
        { label: 'Overdue Work Items', value: `${signals.overdueWorkItems}` },
        ...(risk ? [{ label: 'Top Structured Risk', value: `${risk.title} (${risk.severity})` }] : [])
      ],
      followUps: [
  ...
  if (q.includes('overdue review') || (q.includes('review') && q.includes('overdue'))) {
    return {
      ...
      evidence: [
        { label: 'Open Reviews', value: `${signals.reviewsOpen}` },
        { label: 'Overdue Reviews', value: `${signals.reviewsOverdue}` }
      ],
      followUps: [
  ...
  if (q.includes('blocking') || q.includes('blocked')) {
    return {
      ...
      evidence: [
        { label: 'Blocked Work Items', value: `${signals.blockedWorkItems}` },
        { label: 'Blocked Ratio', value: `${(signals.blockedRatio * 100).toFixed(1)}%` },
        { label: 'Active Work Ratio', value: `${(signals.activeWorkRatio * 100).toFixed(1)}%` }
      ],
      followUps: [
  ...
  return {
    ...
    evidence: risk
      ? [
          { label: 'Top Risk', value: `${risk.title} (${risk.severity})` },
          ...(risk.evidence || []).slice(0, 2).map((entry, index) => ({ label: `Risk Evidence ${index + 1}`, value: entry }))
        ]
