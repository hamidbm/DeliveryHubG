...
const resolveApplication = async (ref: string) => {
  ...
  return {
    app,
    id: String(app._id || app.id || app.aid || id)
  };
};

const ensureApmIndexes = async (db: any) => {
  const safeCreateIndex = async (collection: string, key: any, options?: any) => {
    try {
  ...
  await safeCreateIndex('application_portfolios', { name: 1 }, { unique: true });
  await safeCreateIndex('application_portfolios', { executiveOwner: 1 });

  await safeCreateIndex('application_dependencies', { sourceApplicationId: 1, targetApplicationId: 1, dependencyType: 1 }, { unique: true });
  await safeCreateIndex('application_dependencies', { sourceApplicationId: 1 });
  await safeCreateIndex('application_dependencies', { targetApplicationId: 1 });
  await safeCreateIndex('application_dependencies', { criticality: 1 });

  await safeCreateIndex('application_lifecycle', { applicationId: 1 }, { unique: true });
  await safeCreateIndex('application_lifecycle', { lifecycleStage: 1, updatedAt: -1 });

  await safeCreateIndex('application_environment_strategy', { applicationId: 1 }, { unique: true });

  await safeCreateIndex('release_trains', { name: 1 }, { unique: true });
  await safeCreateIndex('release_trains', { portfolioId: 1, cadence: 1 });

  await safeCreateIndex('applications', { portfolioId: 1 });
  await safeCreateIndex('applications', { releaseTrain: 1 });
  await safeCreateIndex('applications', { lifecycleStatus: 1 });
  await safeCreateIndex('applications', { businessCriticality: 1 });
};

export const ensureApplicationPortfolioIndexes = ensureApmIndexes;

export const listApplicationPortfolios = async () => {
  const db = await getDb();
  await ensureApmIndexes(db);
...
export const listApplicationDependencies = async (applicationId?: string) => {
  ...
  const resolved = applicationId ? await resolveApplication(applicationId) : null;
  const appKey = resolved?.id ? String(resolved.id) : '';
  const query = appKey
    ? { $or: [{ sourceApplicationId: appKey }, { targetApplicationId: appKey }] }
    : {};

  const deps = await db.collection<ApplicationDependency>('application_dependencies').find(query).sort({ createdAt: -1 }).toArray();
  if (!deps.length) return [];

  const ids = Array.from(new Set(deps.flatMap((d) => [toId(d.sourceApplicationId), toId(d.targetApplicationId)]).filter(Boolean)));
  ...
  apps.forEach((app) => {
    const keys = [toId(app._id), toId(app.id), toId(app.aid)].filter(Boolean);
    keys.forEach((key) => appMap.set(key, app));
  });
...
export const createApplicationDependency = async (payload: Partial<ApplicationDependency>) => {
  ...
  const source = await resolveApplication(sourceRef);
  const target = await resolveApplication(targetRef);
...
export const getApplicationEnvironmentStrategy = async (applicationId: string) => {
  ...
  const planning = await db.collection('application_planning_metadata').findOne({
    $or: [
      { scopeType: 'application', scopeId: resolved.id },
      { applicationId: resolved.id }
    ]
  });
...
export const computeApplicationDeliveryImpact = async (applicationId: string) => {
  ...
  const outbound = deps.filter((d) => String(d.sourceApplicationId) === appId);
  const inbound = deps.filter((d) => String(d.targetApplicationId) === appId);

  const connectedIds = Array.from(new Set([
    ...outbound.map((d) => String(d.targetApplicationId)),
    ...inbound.map((d) => String(d.sourceApplicationId))
  ]));
